...
def run_prodigal(sample: dict[str, Any], kind: InputKind, output_dir: Path) -> dict[str, str]:
    ...
    prefix = output_dir / sample["sample_id"]
    protein_path = prefix.with_suffix(".faa")
    cds_path = prefix.with_suffix(".fna")
    gff_path = prefix.with_suffix(".gff")
    mode = "single"
    command = [
        "prodigal",
        "-i",
    ...
    genes = parse_prodigal_gff(gff_path, sample["sample_id"])
    genes_path = prefix.with_suffix(".predicted_genes.json")
    write_json(genes_path, genes)
    genes_tsv = prefix.with_suffix(".predicted_genes.tsv")
    ...
    write_json(
        prefix.with_suffix(".prodigal.json"),
        {
