...
from .storage import ensure_storage_budget, sha256_file
...
PUBLIC_ERROR_CODES = {
    ...
    "invalid_request", "analysis_failed", "legacy_run_failed",
...
def _public_run_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if diagnostic and not result.get("error_code"):
        result["error_code"] = "legacy_run_failed"
            ...
            if not active:
                db.update(
                    "runs", interrupted["id"],
                    {
                        ...
                        "error": "Analysis interrupted: no active worker job remained after QSCN restarted.",
...
def get_run(run_id: str) -> dict[str, Any]:
    ...
    run["interpretations"] = db.list_rows("interpretations", "run_id=?", (run_id,))
    run["capability_rules"] = json.loads(run.pop("capability_rules_json"))
    for interpretation in run["interpretations"]:
        interpretation.pop("result_path", None)
        interpretation["capability_rules"] = json.loads(
            interpretation.pop("capability_rules_json")
        )
        scope = interpretation.pop("analysis_scope_json", None)
        interpretation["analysis_scope"] = json.loads(scope) if scope else all_scope()
    return run
...
def results(run_id: str, interpretation_id: str | None = Query(default=None)) -> dict[str, Any]:
    interpretations = db.list_rows("interpretations", "run_id=?", (run_id,))
    if not interpretations:
        raise _error(404, "results_not_found")
    selected = next((item for item in interpretations if item["id"] == interpretation_id), None)
    selected = selected or interpretations[0]
    return json.loads(Path(selected["result_path"]).read_text(encoding="utf-8"))


@app.get("/api/runs/{run_id}/export")
def export(run_id: str, interpretation_id: str) -> FileResponse:
    try:
        path = build_export(run_id, interpretation_id)
    except ValueError as error:
...
async def create_database(
    ...
    tabular_mode = any(value is not None for value in (hmm_file, pathway_file))
