...
interface Props { results: Results; showSelf: boolean; mode: NetworkViewMode }
...
const NetworkView = forwardRef<NetworkViewHandle, Props>(function NetworkView({ results, showSelf, mode }, ref) {
  const container = useRef<HTMLDivElement>(null)
  const graph = useRef<Core | null>(null)
  const [selected, setSelected] = useState<Record<string, unknown> | null>(null)
  const [tooltip, setTooltip] = useState<Tooltip | null>(null)
  const visibleEvidence = useMemo(() => results.network.edges.filter((edge) => showSelf || !edge.self_communication), [results, showSelf])
  ...
  useEffect(() => {
    ...
    const cy = cytoscape({
      ...
      style: [
        { selector: 'node', style: { label: 'data(label)', 'font-size': '10px', 'font-family': 'Inter, sans-serif', 'font-weight': 600, 'text-wrap': 'wrap', 'text-max-width': '100px', 'text-valign': 'bottom', 'text-margin-y': '7px', color: '#30352f', 'background-color': '#6f9278', 'border-width': 2, 'border-color': '#fdfaf2', width: 'mapData(total_degree, 0, 20, 26, 44)', height: 'mapData(total_degree, 0, 20, 26, 44)' } },
        ...
        {nodeSelected ? <dl>
          ...
          <div className="pathway-evidence-list"><h4>{copy.network.pathwayEvidence}</h4>{evidenceEdges.map((edge) => <article className="pathway-evidence" key={edge.edge_id}>
            <h5>{edge.pathway_name}</h5><code>{edge.pathway_id}</code><dl>
              ...
              <div><dt>{copy.network.fields.senderComponents}</dt><dd>{edge.sender_components.join(', ') || copy.common.unknown}</dd></div>
              <div><dt>{copy.network.fields.receiverComponents}</dt><dd>{edge.receiver_components.join(', ') || copy.common.unknown}</dd></div>
