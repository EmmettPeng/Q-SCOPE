...
test('public v0.3.1 workflow: upload, real worker run, reinterpret, views, downloads, delete', async ({ page }) => {
  const archive = join(tmpdir(), `qscn-e2e-${Date.now()}.zip`)
  const projectName = `v0.3.1 browser E2E ${Date.now()}`
  ...
  await expect(page.getByText('QSCN v0.3.1 · For research use only')).toBeVisible()
