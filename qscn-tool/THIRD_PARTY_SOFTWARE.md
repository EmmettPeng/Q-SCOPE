...
Q-SCOPE builds on FastAPI, Uvicorn, Pydantic, Redis/RQ, React, Cytoscape.js,
GSAP, pdf-lib, Vite, HMMER, Prodigal, Python, Node.js, and the Debian/Alpine base
distributions. Each component remains under its own license.

The Web interface locally bundles Cabinet Grotesk from Fontshare/Indian Type
Foundry. The included WOFF2 remains governed by the ITF Free Font License; it is
loaded only from the packaged application and does not make a runtime request to
Fontshare. Source and license information:
`https://www.fontshare.com/fonts/cabinet-grotesk` and
...
subscription terms. The Q-SCOPE OCI image and launch scripts are distributed by
