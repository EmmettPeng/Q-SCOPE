...
from .schemas import ComponentDefinition, DatabaseManifest
...
from .guidance import guidance_for_database
...
GUIDANCE_TABLE_TEMPLATE = """Format_version\tGuidance_version\tEndpoint_ID\tPathway\tRole\tName\tRequired_profiles\tOptional_profiles\tRecommended_wording\tInterpretation_boundary\tEvidence_IDs\tReferences
1.0\t1.0\tcustom-receptor-core\tExample_receptor_system\treceiving\tReceptor core\tSensorA;RegulatorB\tAccessoryC\tCandidate encoding the receptor-response core\tSequence evidence does not establish expression or activity.\tEVIDENCE-01\thttps://doi.org/10.0000/example
"""

COMPONENT_TABLE_HEADERS = ("Profile_ID", "Display_Name")

COMPONENT_TABLE_TEMPLATE = """Profile_ID\tDisplay_Name
LuxI\tLuxI-like synthase
LuxR\tLuxR-like regulator
"""
...
def parse_component_table(path: Path, profiles: list[str]) -> list[dict[str, str]]:
    try:
        with path.open("r", encoding="utf-8-sig", newline="") as handle:
            rows = list(csv.DictReader(handle, delimiter="\t"))
    except UnicodeDecodeError as error:
        raise DatabaseError(
            "component table must be UTF-8", "database_component_invalid", reason="encoding"
        ) from error
    if not rows or tuple(rows[0].keys()) != COMPONENT_TABLE_HEADERS:
        raise DatabaseError(
            "component table headers do not match the template",
            "database_component_invalid", reason="headers",
        )
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if not profile_id or not display_name:
            raise DatabaseError(
                f"component table row {row_number} has an empty value",
                "database_component_invalid", reason="required_value", row=row_number,
            )
        if profile_id in seen:
            raise DatabaseError(
                f"component table repeats profile {profile_id}",
                "database_component_invalid", reason="duplicate_profile", row=row_number,
            )
        seen.add(profile_id)
        components.append({"profile_id": profile_id, "display_name": display_name})
    expected = set(profiles)
    missing = sorted(expected - seen)
    unknown = sorted(seen - expected)
    if missing or unknown:
        raise DatabaseError(
            "component table must define every HMM profile exactly once",
            "database_component_invalid", reason="profile_mismatch",
            missing_profiles=missing[:10], unknown_profiles=unknown[:10],
        )
    return components


def parse_guidance_table(
    path: Path, pathways: list[dict[str, Any]] | None = None
) -> dict[str, Any]:
    try:
        with path.open("r", encoding="utf-8-sig", newline="") as handle:
            text = handle.read()
        ...
        raise DatabaseError(
            "guidance table must be UTF-8",
            "database_guidance_invalid",
            reason="encoding",
        ) from error
    if "\x00" in text:
        raise DatabaseError(
            "guidance table contains a NUL byte",
            "database_guidance_invalid",
            reason="encoding",
        )
    rows = list(csv.reader(text.splitlines(), delimiter="\t"))
    if not rows or tuple(rows[0]) != GUIDANCE_TABLE_HEADERS:
        raise DatabaseError(
            "guidance table headers do not match the template",
            "database_guidance_invalid",
            reason="headers",
        )
    ...
    for row_number, values in enumerate(rows[1:], start=2):
        if not values or all(not value.strip() for value in values):
            continue
        if len(values) != len(GUIDANCE_TABLE_HEADERS):
            raise DatabaseError(
                f"guidance row {row_number} has the wrong number of columns",
                "database_guidance_invalid",
                reason="columns",
                row=row_number,
            )
        row = {
            header: value.strip()
            for header, value in zip(GUIDANCE_TABLE_HEADERS, values)
        }
        if row["Format_version"] != "1.0":
            raise DatabaseError(
                f"guidance row {row_number} uses an unsupported format version",
                "database_guidance_invalid",
                reason="format_version",
                row=row_number,
            )
        version = row["Guidance_version"]
        endpoint_id = row["Endpoint_ID"]
        if not version or not endpoint_id or safe_id(endpoint_id, "") != endpoint_id:
            raise DatabaseError(
                f"guidance row {row_number} has invalid version or endpoint ID",
                "database_guidance_invalid",
                reason="identity",
                row=row_number,
            )
        if endpoint_id in endpoint_ids:
            raise DatabaseError(
                f"guidance row {row_number} repeats an endpoint ID",
                "database_guidance_invalid",
        ...
        references = _split_table_values(row["References"])
        if any(not reference.startswith(("https://", "http://")) for reference in references):
            raise DatabaseError(
                f"guidance row {row_number} contains an invalid reference URL",
                "database_guidance_invalid",
                reason="reference",
                row=row_number,
            )
        required = _split_table_values(row["Required_profiles"])
        optional = _split_table_values(row["Optional_profiles"])
        if (
            row["Role"] not in {"sending", "receiving"}
            or not row["Pathway"]
            or not row["Name"]
            or not required
            or not row["Recommended_wording"]
            or not row["Interpretation_boundary"]
        ):
            raise DatabaseError(
                f"guidance row {row_number} is missing or has an invalid required value",
                "database_guidance_invalid",
                reason="required_value",
                row=row_number,
            )
        if set(required) & set(optional):
            raise DatabaseError(
                f"guidance row {row_number} overlaps required and optional profiles",
                "database_guidance_invalid",
                reason="overlapping_profiles",
                row=row_number,
            )
        endpoints.append(
            {
                ...
                "wording": row["Recommended_wording"],
                "boundary": row["Interpretation_boundary"],
                "evidence_ids": _split_table_values(row["Evidence_IDs"]),
    ...
    if not endpoints:
        raise DatabaseError(
            "guidance table contains no endpoints",
            "database_guidance_invalid",
            reason="empty_table",
        )
    if len(versions) != 1:
        raise DatabaseError(
            "guidance table mixes guidance versions",
            "database_guidance_invalid",
            reason="mixed_versions",
        )
    guidance = {
        "format_version": "1.0",
    ...
    if pathways is not None:
        validate_guidance_context(guidance, pathways)
    else:
        for endpoint in endpoints:
            endpoint.pop("_source_row")
    return guidance


def validate_guidance_context(
    guidance: dict[str, Any], pathways: list[dict[str, Any]]
) -> None:
    pathway_map = {pathway["pathway_id"]: pathway for pathway in pathways}
    for endpoint in guidance["endpoints"]:
        row = endpoint["_source_row"]
        pathway = pathway_map.get(endpoint["pathway_id"])
        if pathway is None:
            raise DatabaseError(
                f"guidance row {row} references an unknown Pathway",
                "database_guidance_invalid",
                reason="unknown_pathway",
                row=row,
            )
        profiles = pathway[f"{endpoint['role']}_profiles"]
        if len(profiles) <= 1:
            raise DatabaseError(
                f"guidance row {row} targets a single-component role",
                "database_guidance_invalid",
                reason="single_component_role",
                row=row,
            )
        unknown = sorted(
            (set(endpoint["required_profiles"]) | set(endpoint["optional_profiles"]))
            - set(profiles)
        )
        if unknown:
            raise DatabaseError(
                f"guidance row {row} references profiles outside its Pathway role",
                "database_guidance_invalid",
...
def _hmm_profile_summaries(path_value: str, mtime_ns: int) -> tuple[dict[str, Any], ...]:
    del mtime_ns
    ...
    with Path(path_value).open("r", encoding="utf-8", errors="replace") as handle:
        for line in handle:
            ...
            elif current and line.startswith("LENG"):
                current["model_length"] = int(line.split(maxsplit=1)[1])
            elif current and line.startswith("GA"):
                values = line.split(maxsplit=1)[1].replace(";", "").split()
                if values:
                    current["ga_sequence"] = float(values[0])
                    current["ga_domain"] = float(values[1] if len(values) > 1 else values[0])
...
def database_profile_summaries(
    version_id: str, query: str = "", offset: int = 0, limit: int = 50
) -> dict[str, Any]:
    row = get_database(version_id)
    manifest = load_manifest(row)
    hmm_path = Path(row["hmm_path"])
    components = {item.profile_id: item for item in manifest.components}
    usages: dict[str, list[dict[str, str]]] = {}
    for pathway in manifest.pathways:
        for role, profiles in (
            ("sending", pathway.sending_profiles),
            ("receiving", pathway.receiving_profiles),
        ):
    ...
    items = []
    for summary in _hmm_profile_summaries(str(hmm_path), hmm_path.stat().st_mtime_ns):
        component = components.get(summary["profile_id"])
        display_name = component.display_name if component else summary["profile_id"]
        display_label = (
            summary["profile_id"]
            if not component or component.display_name == summary["profile_id"]
            else f"{component.display_name}({summary['profile_id']})"
        )
        items.append({
            **summary,
            "display_name": display_name,
            "display_label": display_label,
            "pathway_roles": usages.get(summary["profile_id"], []),
        })
    needle = query.strip().casefold()
    if needle:
        items = [
            item for item in items
            if needle in item["profile_id"].casefold()
            or needle in item["display_name"].casefold()
            or needle in item["display_label"].casefold()
        ]
    total = len(items)
    return {
        "items": items[offset:offset + limit], "offset": offset, "limit": limit,
        "total": total, "baseline": len(_hmm_profile_summaries(str(hmm_path), hmm_path.stat().st_mtime_ns)),
    }
...
def normalize_legacy_manifest(
    ...
    schema_version = raw.get("schema_version", "1.0") if isinstance(raw, dict) else "1.0"
    components = raw.get("components", []) if schema_version == "2.0" else []
    raw_pathways = raw.get("pathways", {}) if schema_version == "2.0" else raw
    pathways = []
    for pathway_id, items in raw_pathways.items():
...
def validate_manifest_profiles(manifest: DatabaseManifest, hmm_path: Path) -> None:
    ...
    if manifest.schema_version == "2.0" and not manifest.components:
        raise DatabaseError(
            "schema 2.0 requires component definitions",
            "database_component_mismatch",
            missing_profiles=profiles[:10],
        )
    if not manifest.components:
        manifest.components = [
            ComponentDefinition(profile_id=profile, display_name=profile) for profile in profiles
        ]
    component_ids = {component.profile_id for component in manifest.components}
    missing_components = sorted(profile_set - component_ids)
    unknown_components = sorted(component_ids - profile_set)
    if missing_components or unknown_components:
        raise DatabaseError(
            "component definitions must match HMM profiles exactly",
            "database_component_mismatch",
            missing_profiles=missing_components[:10],
            unknown_profiles=unknown_components[:10],
        )
    referenced = {
        profile
        for pathway in manifest.pathways
...
def _store_database(
    ...
    with db.connect() as connection:
        ...
        if existing:
            connection.execute(
                "UPDATE databases SET manifest_path=? WHERE version_id=?",
                (str(manifest_path), existing["version_id"]),
            )
        connection.execute(
...
def register_builtins() -> None:
    ...
    specs = (
        (
            "qsp",
            "QSP published profiles",
            "builtin-v2",
            "Dai et al. QSP database (Water Research, 2023)",
            "QSP published protein-type classification",
            "QSPdatabase.hmm",
            "QSPdatabase.json",
            "ga",
            None,
        ...
        (
            "kegg-m02024",
            "KEGG map02024 QS profiles",
            "m02024-display-v2",
            "KEGG map02024 reference pathway-derived database",
            "KEGG reference architectures with model-system labels",
            "kegg_m02024.hmm",
            "kegg_m02024.json",
            "evalue",
            1e-5,
    ...
    for spec in specs:
        ...
        builtin_guidance = guidance_for_database(db_id)
        if builtin_guidance:
            manifest = DatabaseManifest.model_validate(
                {**manifest.model_dump(mode="json"), "interpretation_guidance": builtin_guidance}
            )
        validate_manifest_profiles(manifest, hmm_path)
        manifest_path = derived_dir / f"{db_id}.{safe_id(version)}.manifest.json"
        ...
        result.append(
            {
                **public,
                ...
                "interpretation_guidance": (
                    manifest.interpretation_guidance.model_dump(mode="json")
...
def import_tabular_database(
    hmm_source: Path,
    pathway_source: Path,
    guidance_source: Path | None = None,
    ...
    *,
    ...
    if guidance_source and guidance_source.suffix.lower() not in {".tsv", ".txt"}:
        raise DatabaseError(
            "interpretation guidance must be a .tsv or .txt file",
            "database_guidance_invalid",
            reason="file_type",
        )
    if component_source and component_source.suffix.lower() not in {".tsv", ".txt"}:
        raise DatabaseError(
            "component names must be a .tsv or .txt file",
            "database_component_invalid", reason="file_type",
        )
    name = name.strip()
    ...
    try:
        ...
        guidance_path = destination / "interpretation-guidance.tsv" if guidance_source else None
        if guidance_source and guidance_path:
            shutil.copyfile(guidance_source, guidance_path)
        component_path = destination / "components.tsv" if component_source else None
        if component_source and component_path:
            shutil.copyfile(component_source, component_path)
        pathways = parse_pathway_table(table_path)
        guidance = parse_guidance_table(guidance_path, pathways) if guidance_path else None
        if guidance and guidance_path:
            guidance["source_sha256"] = sha256_file(guidance_path)
        profiles, _ = hmm_profiles(hmm_path)
        components = (
            parse_component_table(component_path, profiles)
            if component_path else
            [{"profile_id": profile, "display_name": profile} for profile in profiles]
        )
        referenced_profiles = {
            profile
        ...
        manifest = DatabaseManifest.model_validate(
            {
                ...
                "provenance": {
                    ...
                    "guidance_sha256": sha256_file(guidance_path) if guidance_path else None,
                    "guidance_version": guidance["version"] if guidance else None,
                    "component_metadata_sha256": sha256_file(component_path) if component_path else None,
                },
            }
        )
        validate_manifest_profiles(manifest, hmm_path)
        ...
        write_json(manifest_path, manifest.model_dump(mode="json"))
        checksum_paths = [hmm_path, table_path, manifest_path]
        if guidance_path:
            checksum_paths.append(guidance_path)
        if component_path:
            checksum_paths.append(component_path)
        checksum = sha256_paths(checksum_paths)
        return _install_database(destination, manifest, hmm_path, manifest_path, checksum)
    except ValidationError as error:
        params = {"reason": "schema"} if guidance_source else {}
        raise DatabaseError(
            "database guidance is invalid" if guidance_source else "database metadata is invalid",
            "database_guidance_invalid" if guidance_source else "database_metadata_invalid",
            **params,
        ) from error
    except Exception:
        shutil.rmtree(destination, ignore_errors=True)
