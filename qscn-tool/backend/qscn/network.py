    ...
    for pathway_id, roles in grouped.items():
        for sender in roles["sending"]:
            for receiver in roles["receiving"]:
                ...
                annotations = sender.get("biological_annotations", []) + receiver.get(
                    "biological_annotations", []
                )
                annotation_ids = list(dict.fromkeys(
                    annotation["annotation_id"] for annotation in annotations
                ))
                biological_rule_ids = list(dict.fromkeys(
                    annotation["rule_id"] for annotation in annotations
                ))
                biological_states = list(dict.fromkeys(
                    annotation["state"] for annotation in annotations
                ))
                boundary_references = list(dict.fromkeys(
                    reference
                    for annotation in annotations
                    for reference in annotation.get("references", [])
                ))
                edges.append(
                    {
                        ...
                        "sender_component_labels": sender.get("observed_components_labels", sender["observed_components"]),
                        "receiver_component_labels": receiver.get("observed_components_labels", receiver["observed_components"]),
                        "references": references,
                        "annotation_ids": annotation_ids,
                        "biological_rule_ids": biological_rule_ids,
