...
function Wait-QscnReady {
    ...
    for ($attempt = 0; $attempt -lt $maxAttempts; $attempt++) {
        try {
            ...
            if ($result.status -eq 'ready') {
                Write-Host "Q-SCOPE is ready at http://127.0.0.1:$port"
                return
            }
        } catch { }
    ...
    throw "Q-SCOPE did not become ready after $maxAttempts readiness attempts."
}
...
function Backup-Qscn {
    ...
    try {
        & docker run --rm -v 'qscope_data:/source:ro' -v "${target}:/backup" $RedisImage sh -c 'tar -czf /backup/qscn-data.tar.gz -C /source .'
        if ($LASTEXITCODE -ne 0) { throw 'Application-data backup failed.' }
        & docker run --rm -v 'qscope_redis:/source:ro' -v "${target}:/backup" $RedisImage sh -c 'tar -czf /backup/qscn-redis.tar.gz -C /source .'
...
function Restore-Qscn {
    ...
    if (-not (Test-Path (Join-Path $source 'qscn-data.tar.gz')) -or -not (Test-Path (Join-Path $source 'qscn-redis.tar.gz'))) {
        throw 'The selected directory is not a complete Q-SCOPE backup.'
    }
    $confirmation = Read-Host 'This replaces the current Q-SCOPE volumes. Type RESTORE to continue'
    if ($confirmation -ne 'RESTORE') { Write-Host 'Restore cancelled.'; return }
    Invoke-Compose down
    & docker volume create qscope_data *> $null
    & docker volume create qscope_redis *> $null
    & docker run --rm -v 'qscope_data:/target' -v "${source}:/backup:ro" $RedisImage sh -c 'find /target -mindepth 1 -maxdepth 1 -exec rm -rf -- {} + && tar -xzf /backup/qscn-data.tar.gz -C /target'
    if ($LASTEXITCODE -ne 0) { throw 'Application-data restore failed.' }
    & docker run --rm -v 'qscope_redis:/target' -v "${source}:/backup:ro" $RedisImage sh -c 'find /target -mindepth 1 -maxdepth 1 -exec rm -rf -- {} + && tar -xzf /backup/qscn-redis.tar.gz -C /target'
