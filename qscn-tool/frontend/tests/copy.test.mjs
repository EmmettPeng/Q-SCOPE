...
test('machine values receive stable human-readable display labels', () => {
  ...
  assert.equal(module.displayRuleStrategy('required_profiles'), 'Selected required components')
...
test('v1.0.0 release identity and potential-network title are frozen', () => {
  assert.equal(module.copy.common.product, 'Quorum-Sensing Communication Link Predictor (Q-SCOPE)')
  assert.equal(module.copy.common.release, 'Q-SCOPE v1.0.0 · For research use only')
  assert.equal(module.copy.network.title, 'Quorum-sensing communication network (potential)')
})
