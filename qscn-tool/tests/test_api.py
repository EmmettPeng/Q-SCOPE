...
from qscn.api import (
    ...
    create_database,
    create_run,
    database_component_template,
    database_pathway_template,
    database_guidance_template,
    health,
    project_analysis_tree,
    reconcile_interrupted_runs,
    rename_interpretation,
)
from qscn.config import Settings
from qscn.schemas import DatabaseManifest, RenameInterpretationRequest, RunRequest
...
class ReleaseSchemaTest(unittest.TestCase):
    def test_database_api_accepts_new_two_file_mode_and_legacy_zip(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            component_bytes = b"Profile_ID\tDisplay_Name\nA\tAlpha protein\n"
            legacy_manifest = {
            ...
            with (
                ...
                tabular = asyncio.run(create_database(
                    ...
                    guidance_file=None,
                    component_file=UploadFile(filename="components.tsv", file=io.BytesIO(component_bytes)),
                ...
                legacy = asyncio.run(create_database(
                    ...
                    hmm_file=None, pathway_file=None, guidance_file=None, component_file=None, name=None, version=None, source=None,
            ...
            self.assertEqual(tabular["components"], [{"profile_id": "A", "display_name": "Alpha protein"}])
    ...
    def test_guidance_template_is_downloadable_and_versioned(self):
        response = database_guidance_template()
        content = response.body.decode("utf-8")
        self.assertIn("qscn-interpretation-guidance-template.tsv", response.headers["content-disposition"])
        self.assertEqual(content.splitlines()[0].split("\t")[:3], [
            "Format_version", "Guidance_version", "Endpoint_ID",
        ])
        self.assertTrue(content.splitlines()[1].startswith("1.0\t"))

    def test_component_template_is_downloadable(self):
        response = database_component_template()
        content = response.body.decode("utf-8")
        self.assertIn("qscn-component-names-template.tsv", response.headers["content-disposition"])
        self.assertEqual(content.splitlines()[0], "Profile_ID\tDisplay_Name")
        self.assertIn("LuxI\tLuxI-like synthase", content)
    ...
    def test_incompatible_schema_is_rejected_without_mutation(self):
        with tempfile.TemporaryDirectory() as temp:
            settings = test_settings(Path(temp))
            with patch("qscn.db.settings", settings):
                with db.connect() as connection:
                    connection.execute("CREATE TABLE application_metadata (key TEXT PRIMARY KEY, value TEXT NOT NULL)")
                    connection.execute("INSERT INTO application_metadata VALUES ('schema_version','999')")
                    connection.execute("CREATE TABLE projects (id TEXT PRIMARY KEY)")
                    connection.execute("INSERT INTO projects VALUES ('preserved')")
                with self.assertRaisesRegex(db.SchemaVersionError, "Unsupported Q-SCOPE schema 999"):
                    db.initialize()
                with db.connect() as connection:
                    self.assertEqual(connection.execute("SELECT id FROM projects").fetchone()["id"], "preserved")
    ...
    def test_analysis_tree_groups_runs_and_renames_interpretations(self):
        with tempfile.TemporaryDirectory() as temp:
            settings = test_settings(Path(temp))
            with patch("qscn.db.settings", settings):
                db.initialize()
                ...
                for database_id in ("qsp:v1", "kegg:v1"):
                    db.insert("databases", {
                        "version_id": database_id, "database_id": database_id.split(":")[0],
                        "name": database_id.split(":")[0].upper(), "version": "1", "source": "test",
                        "classification_scheme": "test", "checksum": database_id, "hmm_path": "/hmm",
                        "manifest_path": "/manifest", "threshold_type": "ga", "default_evalue": None,
                        "provenance_json": "{}", "builtin": 1, "created_at": timestamp,
                    })
                for run_id, database_id in (("run-qsp-1", "qsp:v1"), ("run-qsp-2", "qsp:v1"), ("run-kegg", "kegg:v1")):
                    db.insert("runs", {
                        "id": run_id, "project_id": "project", "database_version_id": database_id,
                        "input_kind": "protein", "completeness_mode": "v2",
                        "capability_rule_version": "3.0", "capability_rules_json": "[]",
                        "sequence_evalue": None, "hit_rule_version": "1.0", "hit_thresholds_json": None,
                        "analysis_scope_version": "1.0", "analysis_scope_json": json.dumps({
                            "version": "1.0", "mode": "all", "requested_values": [], "resolved_pathway_ids": [],
                        }), "status": "complete", "stage": "complete", "progress": 1,
                        "rq_job_id": None, "error": None, "error_code": None, "error_params_json": None,
                        "created_at": timestamp, "updated_at": timestamp,
                    })
                result_dir = settings.projects_dir / "project" / "runs" / "run-qsp-1" / "interpretations" / "i1"
                result_dir.mkdir(parents=True)
                (result_dir / "results.json").write_text("{}", encoding="utf-8")
                for interpretation_id, run_id in (("i1", "run-qsp-1"), ("i2", "run-qsp-1"), ("i3", "run-kegg")):
                    path = result_dir / "results.json"
                    db.insert("interpretations", {
                        "id": interpretation_id, "run_id": run_id, "completeness_mode": "v2",
                        "capability_rule_version": "3.0", "capability_rules_json": "[]",
                        "analysis_scope_version": "1.0", "analysis_scope_json": json.dumps({
                            "version": "1.0", "mode": "all", "requested_values": [], "resolved_pathway_ids": [],
                        }), "name": None, "result_path": str(path), "created_at": timestamp, "updated_at": timestamp,
                    })
                tree = project_analysis_tree("project")
                self.assertEqual(len(tree["databases"]), 2)
                self.assertEqual(sum(len(item["runs"]) for item in tree["databases"]), 3)
                qsp = next(item for item in tree["databases"] if item["database_version_id"] == "qsp:v1")
                self.assertEqual(sum(len(run["interpretations"]) for run in qsp["runs"]), 2)
                renamed = rename_interpretation("run-qsp-1", "i1", RenameInterpretationRequest(name=" Uptake endpoint "))
                self.assertEqual(renamed["display_name"], "Uptake endpoint")
                self.assertEqual(db.get("interpretations", "i1")["name"], "Uptake endpoint")
            ...
            with patch("qscn.db.settings", settings):
                ...
                for run_id, job_id in (("orphan", "gone"), ("active", "live")):
                    db.insert("runs", {
                        ...
                        "input_kind": "protein", "capability_rule_version": "3.0",
